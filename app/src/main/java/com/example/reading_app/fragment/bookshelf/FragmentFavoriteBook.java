...
public class FragmentFavoriteBook extends Fragment implements RecyclerViewBookAdapter.BookItemListener {
    ...
    private void getBookForUser() {
        ...
        if (user != null) {
            ApiService.apiService.getFavoriteBookByUser(user.getUsername()).enqueue(new Callback<List<BookResonseDTO>>() {
                @Override
